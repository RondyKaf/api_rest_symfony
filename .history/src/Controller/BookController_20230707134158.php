...
class BookController extends AbstractController
{
    ...
    public function getDetailBooks(int $id,BookRepository $bookRepository,SerializerInterface $serializer):JsonResponse
    {
        ...
        if($detailBooks){
            $jsonBook = $serializer->serialize($detailBooks,'json',$context);
    ...
    public function updateBooks(Request $request,EntityManagerInterface $em,SerializerInterface $serializer,Book $currentBook,ValidatorInterface $validate):JsonResponse
    {
        $newBook = $serializer->deserialize($request->getContent(), Book::class, 'json');

        $currentBook->setTitle($newBook->getTitle());
        $currentBook->setCoverTitle($newBook->getCoverTitle());

        $errors = $validate->validate($currentBook);
        if($errors->getContent() > 0){
            return new JsonResponse($serializer->serialize($errors,'json',JsonResponse::HTTP_BAD_REQUEST,[],ture))
        }
